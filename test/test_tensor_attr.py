...
def test_constant_of_shape_mlir_content(iree_device):
    ...
    Without tensor attribute support, the MLIR would contain "NYI" and fail.
    """
    model = _make_dynamic_constant_of_shape_model(0.0)
    model_dir = tempfile.mkdtemp()
    model_path = os.path.join(model_dir, "model.onnx")
    ...
    try:
        ...
        # Correctness: ConstantOfShape(0.0) + X = X
        ...
        assert new_mlir, "No MLIR files saved"

        mlir_content = open(list(new_mlir)[0]).read()

        # ConstantOfShape should have torch.onnx.value = dense<...> : tensor<...>
        # NOT torch.onnx.value = "NYI"
        assert (
            '"NYI"' not in mlir_content
        ), "MLIR contains 'NYI' — tensor attribute was not properly serialized"
        assert (
