...
// 2D grid kernel: computes linear thread ID from a 2D grid.
//
// Identical to sigmoid_extern in computation, but uses both blockIdx.x and
// blockIdx.y to compute the thread ID. This means the result is WRONG if
// the Y workgroup count is incorrect — validating that all workgroup_count
// dimensions are passed correctly through hal.dispatch.extern.
//
//   tid = (blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x + threadIdx.x
//
// ABI contract (same as sigmoid_extern):
//   Bindings: binding0 (ReadOnly) input float[N], binding1 output float[N]
//   Push constants: N (i32)
extern "C" __global__ void sigmoid_2d_extern(const float* __restrict__ input,
