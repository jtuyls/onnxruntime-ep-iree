...
to exercise multi-workgroup dispatch. Three test models:
  ...
  4. All-Dynamic Workgroup Count (X, Y, Z all via $N refs):
     Same as (3) but with wg_y=1, wg_z=1 also as dynamic $N inputs:
     ExternDispatch(relu_out, n_elements, wg_x, wg_y, wg_z,
                    push_constants=["$1"], workgroup_count=["$2","$3","$4"])
...
# ---------------------------------------------------------------------------
# Test 4: All-Dynamic Workgroup Count (X, Y, Z all via $N refs)
#   input(256xf32) -> Relu -> relu_out
#   Shape(input) -> Gather(0) -> n_elements (scalar i64)
#   ceildiv(n_elements, WG_SIZE) -> wg_x
#   Constant(1) -> wg_y, wg_z (scalar i64, computed dynamically)
#   ExternDispatch(relu_out, n_elements, wg_x, wg_y, wg_z,
#                  push_constants=["$1"], workgroup_count=["$2","$3","$4"])
# ---------------------------------------------------------------------------


def create_all_dynamic_sigmoid_model(target_arch):
    """Sigmoid with all three workgroup_count dims as dynamic $N references."""
    ko = f"kernel_{target_arch}.co"
    nodes = [
        helper.make_node("Relu", ["input"], ["relu_out"]),
        # Compute n_elements = Shape(input)[0]
        ...
        helper.make_node("Add", ["n_elements", "const_wg_m1"], ["n_plus_wg_m1"]),
        helper.make_node("Div", ["n_plus_wg_m1", "const_wg"], ["wg_x"]),
        # wg_y and wg_z as dynamic scalar values (both 1)
        helper.make_node("Constant", [], ["wg_y"], value=_make_i64_scalar("", 1)),
        helper.make_node("Constant", [], ["wg_z"], value=_make_i64_scalar("", 1)),
        # ExternDispatch with all three workgroup dims dynamic
        helper.make_node(
            "ExternDispatch",
            inputs=["relu_out", "n_elements", "wg_x", "wg_y", "wg_z"],
            outputs=["output"],
            name="ext_all_dynamic",
            domain="com.iree",
            kernel_name="sigmoid_extern",
            kernel_object=ko,
            workgroup_size=[WG_SIZE, 1, 1],
    ...
    return build_model(
        ...
        value_info=[
            ...
            helper.make_tensor_value_info("n_plus_wg_m1", I64, []),
            helper.make_tensor_value_info("wg_x", I64, []),
            helper.make_tensor_value_info("wg_y", I64, []),
            helper.make_tensor_value_info("wg_z", I64, []),
        ],
    )


def all_dynamic_sigmoid_expected(input_data):
    after_relu = np.maximum(input_data, 0)
    return 1.0 / (1.0 + np.exp(-after_relu))
...
class TestDynamicDispatchE2E:
    ...
    def test_all_dynamic_workgroup_count(self, gpu_device, kernel_dir, target_arch):
        """All three workgroup_count dims (X, Y, Z) via $N input refs."""
        rng = np.random.default_rng(42)
        input_data = rng.standard_normal(N).astype(np.float32)
        model = create_all_dynamic_sigmoid_model(target_arch)
        expected = all_dynamic_sigmoid_expected(input_data)
...
def main():
    ...
    if args.model_only:
        ...
        onnx.save(
            create_all_dynamic_sigmoid_model(args.target_arch),
            str(out / "all_dynamic_sigmoid_e2e.onnx"),
        )
    ...
    # Test 4: All-Dynamic Workgroup Count (X, Y, Z all via $N refs)
    print(
        f"\n=== Test 4: All-Dynamic Workgroup Count ($N refs for X/Y/Z, {N} elements) ==="
    )
    rng4 = np.random.default_rng(42)
    input_data4 = rng4.standard_normal(N).astype(np.float32)
    expected4 = all_dynamic_sigmoid_expected(input_data4)
    try:
        result4 = run_model(
            create_all_dynamic_sigmoid_model(args.target_arch),
            {"input": input_data4},
        ...
        print(f"  Shape: {input_data4.shape}")
        print(f"  First 4 expected: {expected4[:4]}")
        print(f"  First 4 got:      {result4[:4]}")
        if np.allclose(result4, expected4, atol=1e-3):
            print("  PASSED")
            passed += 1
        else:
            print(f"  FAILED: max diff = {np.max(np.abs(result4 - expected4))}")
            failed += 1
    except Exception as e:
        print(f"  FAILED: {e}")
        failed += 1
